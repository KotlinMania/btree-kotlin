...
/**
 * Drop files from a Codebase whose path matches a `reexport_modules` pattern
 * loaded from `.ast_distance_config.json`.
 *
 * For source codebases the match is against the file's relative path (so
 * `mod.rs` / `lib.rs` get filtered out because they are module-wiring, not
 * portable units). For target codebases we additionally inspect each file's
 * `// port-lint: source <path>` header — if that header points at a forbidden
 * source file, the target counterpart is also dropped so it does not appear in
 * the matched-files dashboard as a phantom STUB.
 */
static void apply_reexport_filter(Codebase& cb) {
    if (!g_reexport_config.loaded || g_reexport_config.patterns.empty()) return;

    std::vector<std::string> to_drop;
    to_drop.reserve(cb.files.size());

    for (const auto& [key, sf] : cb.files) {
        if (g_reexport_config.matches(sf.relative_path)) {
            to_drop.push_back(key);
            continue;
        }

        // Target side: also drop Kotlin/etc. files whose port-lint header
        // points at a source file that itself is a reexport-only module.
        if (cb.language != "rust" && !sf.paths.empty()) {
            std::string header_src;
            try {
                std::ifstream in(sf.paths.front());
                std::string line;
                int lines_checked = 0;
                while (lines_checked < 10 && std::getline(in, line)) {
                    ++lines_checked;
                    const std::string marker = "port-lint: source ";
                    auto pos = line.find(marker);
                    if (pos != std::string::npos) {
                        header_src = line.substr(pos + marker.size());
                        // Trim trailing whitespace/newline.
                        while (!header_src.empty() &&
                               (header_src.back() == ' ' || header_src.back() == '\r' ||
                                header_src.back() == '\t' || header_src.back() == '\n')) {
                            header_src.pop_back();
                        }
                        break;
                    }
                ...
                // Best-effort read; ignore failures.
            }
            if (!header_src.empty() && g_reexport_config.matches(header_src)) {
                to_drop.push_back(key);
            }
        }
    }
    ...
    for (const auto& key : to_drop) {
        auto it = cb.files.find(key);
        if (it == cb.files.end()) continue;
        const std::string stem = it->second.stem;
        const std::string qn = it->second.qualified_name;
        cb.files.erase(it);

        auto stem_it = cb.by_stem.find(stem);
        if (stem_it != cb.by_stem.end()) {
            auto& v = stem_it->second;
            v.erase(std::remove(v.begin(), v.end(), key), v.end());
            if (v.empty()) cb.by_stem.erase(stem_it);
        }

        auto qn_it = cb.by_qualified.find(qn);
        if (qn_it != cb.by_qualified.end() && qn_it->second == key) {
            cb.by_qualified.erase(qn_it);
        }
    }

    std::cerr << "Info: filtered " << to_drop.size()
              << " reexport-module file" << (to_drop.size() == 1 ? "" : "s")
              << " from " << cb.language << " codebase per "
              << g_reexport_config.config_path << "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // Swarm task management commands
            ...
            // Stubs in the *source* are treated as baseline. When BOTH sides have stub markers we treat the
            // target as a faithful transliteration of an upstream open issue (FIXME/TODO), not as a porting
            // failure — only target-only stub markers force the score to zero.
            if (file2_stubs && !file1_stubs) {
                content_score = 0.0f;
                std::cout << "\n*** STUB DETECTED ***\n";
                ...
                if (file1_stubs && file2_stubs) {
                    std::cout << "\nNote: both " << file1 << " and " << file2
                              << " contain TODO/stub/placeholder markers; treating as faithful transliteration.\n";
                } else if (file1_stubs) {
                    std::cout << "\nNote: " << file1
