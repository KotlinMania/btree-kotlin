    ...
    void compute_similarities() {
        ...
        for (auto& m : matches) {
            try {
                ...
                bool tgt_has_stubs = parser.has_stub_bodies_in_files(
                    tgt_file.paths, tgt_lang);
                bool src_has_stubs = tgt_has_stubs &&
                    parser.has_stub_bodies_in_files(src_file.paths, src_lang);

                // Target stub markers force the score to zero only when the SOURCE
                // does not have matching markers. A FIXME/TODO that exists in the
                // upstream Rust source and was faithfully carried into the Kotlin
                // port is a transliterated open issue, not a porting failure.
                if (tgt_has_stubs && !src_has_stubs) {
                    m.similarity = 0.0f;
                    m.is_stub = true;
                } else {
